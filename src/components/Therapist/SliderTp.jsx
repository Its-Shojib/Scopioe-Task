...
export default function SliderTp({ therapists }) {
    ...
    <div className="slider-container">
      <Slider ref={(c) => setSlider(c)} {...settings}>
        {therapists?.map((item, index) => (
          <div key={index} className="border border-gray-200 shadow-xl rounded-lg">
            <img className="w-full h-[220px] p-3" src={item.image} alt={item.name} />
            <div className="px-5 font-poppins">
              <h2 className="text-lg text-[#152A16] font-medium mt-3">{item.name}</h2>
              ...
              <div className="flex justify-start items-center gap-2 mb-6">
                <FaCar />
                <p className="card-text text-[#5C635A] text-sm">{item.service}</p>
              </div>
            </div>
